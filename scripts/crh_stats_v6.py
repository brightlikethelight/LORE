#!/usr/bin/env python3
"""Statistical significance tests for CRH v6 evaluation."""
...
async def main():
    ...
    try:
        ...
        logger.info(f"Wilcoxon p-value: {wilcoxon_p:.4f}")
        ...
        non_zero = [(i, d) for i, d in enumerate(differences) if d != 0]
        if non_zero:
    ...
    if wilcoxon_p is not None:
        logger.info(f"Wilcoxon test (suspicion scores): p = {wilcoxon_p:.4f} {'*' if wilcoxon_significant else ''}")
    if t_p is not None:
        logger.info(f"Paired t-test (suspicion scores): p = {t_p:.4f} {'*' if ttest_significant else ''}")
